"""Parallel, causal daily strategy checks used by the D1 market scan.

This module intentionally evaluates report/alert dimensions only.  It does not
participate in the Turtle detector state machine, order generation, or
...
from inv_trend.core.signals import crossed_above, crossed_below
...
def analyze_prepared_strategy_checks(
    ...
    *,
    ...
    for name, indicator, parameters in (
        ("sma_alignment", current["sma_alignment"], {"periods": list(prepared.config.sma_periods)}),
        ("ema_trend", current["ema_trend"], {"periods": list(prepared.config.ema_periods)}),
    ):
...
def _state_at(prepared: PreparedDailyStrategyChecks, position: int) -> dict[str, Any]:
    ...
    rating = _rating(turtle, sma, ema, macd_summary, trend_quality, volatility, volume, prepared.config)
    return {
        "sma_alignment": sma,
        "ema_trend": ema,
        "macd": macd,
...
def _sma_alignment(row: pd.Series, config: DailyChecksConfig) -> dict[str, Any]:
...
def _ema_trend(
    row: pd.Series, config: DailyChecksConfig, completed_bars: int
) -> dict[str, Any]:
    short, long = config.ema_periods
    short_value = _finite_float(row.get(f"ema_{short}"))
    long_value = _finite_float(row.get(f"ema_{long}"))
    # EMA starts recursively at bar one, but this strategy deliberately waits
    # for its longest configured period before exposing a direction.
...
def _macd_state(frame: pd.DataFrame, config: DailyChecksConfig) -> dict[str, Any]:
    minimum = config.macd_slow + config.macd_signal - 1
    cross_minimum = minimum + 1
    if len(frame) < minimum:
        return {
            "status": "unavailable", "cross_status": "unavailable", "bar_end": None,
            "dif": None, "dea": None, "histogram": None, "direction": None,
            "zero_axis": None, "event": None, "cross_direction": None,
        }
    row = frame.iloc[-1]
    ...
    zero_axis = "long" if dif > 0.0 and dea > 0.0 else ("short" if dif < 0.0 and dea < 0.0 else "none")
    return {
        "status": "ready",
...
def _macd_summary(macd: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    values = [item["zero_axis"] for item in macd.values() if item.get("zero_axis") in {"long", "short"}]
    ...
    if len(directions) == 1:
        ...
        direction = "none"
    return {"status": "ready" if values else "unavailable", "direction": direction, "active_timeframes": [label for label, item in macd.items() if item.get("zero_axis") in {"long", "short"}]}


def _trend_quality(row: pd.Series, config: DailyChecksConfig) -> dict[str, Any]:
    ...
    if None in (adx, plus_di, minus_di):
        return {"status": "unavailable", "direction": None, "adx": None, "plus_di": None, "minus_di": None, "confirmed": False}
    return {
        "status": "ready",
...
def _volatility(row: pd.Series, config: DailyChecksConfig) -> dict[str, Any]:
    ...
    if None in (atr, atr_pct, percentile):
        return {"status": "unavailable", "atr": atr, "atr_pct": atr_pct, "percentile": percentile, "state": None}
    if config.atr_normal_percentile_low <= float(percentile) <= config.atr_normal_percentile_high:
        ...
        state = "high"
    return {"status": "ready", "atr": atr, "atr_pct": atr_pct, "percentile": percentile, "state": state}


def _volume(row: pd.Series, config: DailyChecksConfig) -> dict[str, Any]:
    ...
    if None in (relative, baseline, current):
        return {"status": "unavailable", "volume": current, "baseline": baseline, "relative_volume": relative, "confirmed": False}
    return {
        "status": "ready",
        "volume": current,
        "baseline": baseline,
...
def _rating(
    turtle: Mapping[str, Any], sma: Mapping[str, Any], ema: Mapping[str, Any],
    macd: Mapping[str, Any], trend: Mapping[str, Any], volatility: Mapping[str, Any],
    volume: Mapping[str, Any], config: DailyChecksConfig,
) -> dict[str, Any]:
    family_votes = {
    ...
    if len(directional) > 1 or any(value == "conflict" for value in family_votes.values()):
        return {"grade": "CONFLICT", "direction": None, "score": 0.0, "family_votes": family_votes, "aligned_families": [], "quality_adjustments": []}
    if not directional:
        return {"grade": "NONE", "direction": None, "score": 0.0, "family_votes": family_votes, "aligned_families": [], "quality_adjustments": []}
    ...
    if len(aligned) >= config.rating_a_min_families and score >= config.rating_a_min_score:
        ...
        grade = "C"
    return {"grade": grade, "direction": direction, "score": score, "family_votes": family_votes, "aligned_families": aligned, "quality_adjustments": adjustments}


def _unavailable_alignment(periods: tuple[int, ...]) -> dict[str, Any]:
    return {"status": "unavailable", "direction": None, "periods": list(periods), "values": {str(period): None for period in periods}, "reference_value": None}
...
def _empty_checks(config: DailyChecksConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "macd": {label: _macd_state(pd.DataFrame(), config) for label in (_timeframe_label(value) for value in config.macd_session_periods)},
        "macd_summary": {"status": "unavailable", "direction": None, "active_timeframes": []},
        "trend_quality": {"status": "unavailable", "direction": None, "adx": None, "plus_di": None, "minus_di": None, "confirmed": False},
        "volatility": {"status": "unavailable", "atr": None, "atr_pct": None, "percentile": None, "state": None},
        "volume": {"status": "unavailable", "volume": None, "baseline": None, "relative_volume": None, "confirmed": False},
        "rating": {"grade": "NONE", "direction": None, "score": 0.0, "family_votes": {}, "aligned_families": [], "quality_adjustments": []},
        "signals": [],
    }
