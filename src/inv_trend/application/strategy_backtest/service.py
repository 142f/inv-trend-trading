"""Batch orchestration for the independent strategy-backtest stage."""
...
class StrategyBacktestService:
    """Run one deterministic grid against one immutable source/data bundle."""
    ...
    def run(
        ...
        *,
        ...
        projector = StrategyReplayProjector(source)
...
def _aggregate_fold_metrics(
    ...
    if len(finite_cagrs):
        ...
        aggregate["cagr_iqr_ratio"] = dispersion
...
def _rank_results(
    ...
    for result in results:
        ...
        score = (
            plan.ranking.sharpe_weight * percentiles["sharpe"][result.combination_id]
            + plan.ranking.mar_weight * percentiles["mar"][result.combination_id]
            + plan.ranking.cagr_weight * percentiles["cagr"][result.combination_id]
    ...
    holdout_order = sorted(
        ...
        key=lambda item: (
            _number(item.holdout_metrics.get("sharpe")) or -math.inf,
            _number(item.holdout_metrics.get("cagr")) or -math.inf,
        ), reverse=True,
    )
...
def _overfit_risk(
    ...
    train_sharpe_values = [
        _number(fold.train_metrics.get("sharpe")) for fold in item.folds
    ]
    ...
    oos_sharpe = _number(item.validation_metrics.get("sharpe"))
...
__all__ = ["StrategyBacktestService"]
