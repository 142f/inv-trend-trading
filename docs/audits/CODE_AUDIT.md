...
> **Current-path note (2026-08-18):** this is a historical audit. Its former
> top-level package names now map to `src/inv_trend/`; current CLI modules are
> in `inv_trend.cli` and the remaining Turtle implementations are under
> `inv_trend.adapters`.
...
- `inv_trend.cli.data`: data build, metal-tech build/backtest, US trend
  alerts.
- `inv_trend.adapters.multi_asset.us_trend_alerts`: standalone alert scanner.
